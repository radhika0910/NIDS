...
mongoose.connect('mongodb+srv://bhoyarradhika17:ltCjANFllcxYTO3g@clusterdl.sofuy.mongodb.net/local?retryWrites=true&w=majority&appName=ClusterDl', {
...
app.get("/csv", function(req, res) {
  // Simulate user is always authenticated
  res.render("csv");
});
...
app.get("/login", function(req, res) {
  // Create a dummy user object with a static user ID or username
  const dummyUser = {
    _id: "staticUserId",  // You can set any static or dummy ID
    username: "staticUsername"
  };

  // Manually log in the dummy user
  req.login(dummyUser, function(err) {
    if (err) {
      console.log(err);
    } else {
      // Redirect the user after "successful" login
      res.redirect("/submit");  // Change this to the page you want the user to go to after login
    }
  });
});
